...
/**
 * Servlet implementation class DescifrarVigenere
 */
@WebServlet("/DescifrarVigenere")
public class DescifrarVigenere extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		this.mostrarFormularioHtml(request, response, "", "", "", "", "", false, 5);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case "Descargar CSV":
			...
			// filename=\"libros.csv\"");
			salida.println(DocumentoCsv.deLibros(mensajeCifrado, 3));
			break;
		case "Descargar XML":
			response.setCharacterEncoding("UTF-8");
			...
			// response.setHeader("Content-Disposition", "attachment;
			// filename=\"libros.xml\"");
			salida2.println(DocumentoXml.deLibros(mensajeCifrado, 3));
			break;
		case "Descargar JSON":
			response.setCharacterEncoding("UTF-8");
			...
			salida3.println(DocumentoJson.deLibros(mensajeCifrado, 3));
			break;
		case "Descargar XLS":
			/* Microsoft Excel Worksheet */
			DocumentoXls.deLibros(response, mensajeCifrado, 3);
			break;
		case "Descargar XLSX":
			/* Microsoft 2007 spreadsheet */
			DocumentoXlsx.deLibros(response, mensajeCifrado, 3);
			break;
		case "Descargar PDF":
			/* PDF Document */
			DocumentoPdf.deLibros(response, mensajeCifrado, 3);
			break;
		default:
	...
	private void procesarFormularioHtml(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			if (e1.length() > 0 || e2.length() > 0) {
				this.mostrarFormularioHtml(request, response, "", "", clave, e1, e2, false, 5);
			}
			...
			if (band) {
				...
				this.mostrarFormularioHtml(request, response, mensaje, cifrado, clave, "", "", true, 6);
			}
	...
	protected String cifrar(String message, String clv) {
		...
		for (int a = 0; a < mensaje.length; a++) {
			...
			caracter = obtenerNuevoCaracter(mensaje[a], clave[claveIndex], true);
